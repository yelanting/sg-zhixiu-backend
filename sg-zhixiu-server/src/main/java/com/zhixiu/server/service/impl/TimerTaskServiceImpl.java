...
import com.zhixiu.server.dao.TimerTaskMapper;
import com.zhixiu.server.definition.form.TimerTaskFormDefinition;
import com.zhixiu.server.model.TimerTask;
...
import com.zhixiu.server.service.TimerTaskService;
import com.zhixiu.server.tools.QuartzManager;
import com.zhixiu.server.tools.TaskDefine;
...
public class TimerTaskServiceImpl implements TimerTaskService {
     ...
     * 添加测试环境地址信息实现方法
     * 
     * @see com.administrator.platform.service.TimerTaskService#addTimerTask(com.administrator.platform.model.TimerTask)
     */
    @Override
    ...
    @Override
    public int save(TimerTask record) {
        if (null == record || null == record.getId() || 0 == record.getId()) {
            addTimerTask(record);
        } else {
            updateTimerTask(record);
        }
    ...
    @Override
    public int delete(TimerTask record) {
        timerTaskMapper.deleteByPrimaryKey(record.getId());
        return 0;
    }

    @Override
    public int delete(List<TimerTask> records) {
        for (TimerTask timerTask : records) {
            delete(timerTask);
        }
        return 0;
    }
    ...
    @Override
    public TimerTask findById(Long id) {
        return getTimerTaskById(id);
    }
    ...
    public PageResult findPage(PageRequest pageRequest) {
        ...
        if (label != null && label.getValue() != null) {
            return MybatisPageHelper.findPage(pageRequest, timerTaskMapper,
    ...
    @Override
    public TimerTask insertOrUpdate(TimerTask record) {
        if (save(record) > 0) {
            return record;
        }
