...
import com.zhixiu.gateway.constant.TimerTaskAndRelations;
import com.zhixiu.gateway.dao.TimerTaskPolicyMapper;
import com.zhixiu.gateway.definition.form.TimerTaskPolicyFormDefinition;
import com.zhixiu.gateway.model.TimerTaskPolicy;
...
import com.zhixiu.gateway.service.TimerTaskPolicyService;
 ...
 * @see :
 */
@Service("timerTaskPolicyService")
public class TimerTaskPolicyServiceImpl implements TimerTaskPolicyService {
	 ...
	 * 添加测试环境地址信息实现方法
	 * 
	 * @see com.administrator.platform.service.GoodService#addGood(com.administrator.platform.model.Good)
	 */
	@Override
	...
	@Override
	public TimerTaskPolicy insertOrUpdate(TimerTaskPolicy timerTaskPolicy) {
		if (null == timerTaskPolicy.getId() || timerTaskPolicy.getId() == 0) {
			return addTimerTaskPolicy(timerTaskPolicy);
		}

		return updateTimerTaskPolicy(timerTaskPolicy);
	}

	@Override
	public int save(TimerTaskPolicy record) {
		if (null != insertOrUpdate(record)) {
			return 0;
		}
	...
	@Override
	public int delete(TimerTaskPolicy record) {
		return deleteTimerTaskPolicy(record.getId());
	}

	@Override
	public int delete(List<TimerTaskPolicy> records) {
		return deleteTimerTaskPolicy(
		        records.stream().map(eachItem -> eachItem.getId())
		                .collect(Collectors.toList()).toArray(new Long[] {}));
	}

	@Override
	public TimerTaskPolicy findById(Long id) {
		return getTimerTaskPolicyById(id);
	}

	@Override
	public PageResult findPage(PageRequest pageRequest) {
		Param label = pageRequest.getParam("cname");
		if (label != null && label.getValue() != null) {
			return MybatisPageHelper.findPage(pageRequest,
			        timerTaskPolicyMapper, "findPageByCname", label.getValue());
		}
		return MybatisPageHelper.findPage(pageRequest, timerTaskPolicyMapper);
	}
}
